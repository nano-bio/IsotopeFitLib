...
void test2() {
    ...
    uint16_t combo3 = (hue[124] << 8 | hue[125]);
...
//    std::cout << std::endl;
    ...
    char *asdf = new char[8];
    
    for(int i=0; i< 8; i++)
    {
        asdf[i] = uncompressed[i+8263];
...
void test3() {
    ...
    ReadMatBinFile(fs, hue);
    
    std::cout << (sizeof(hue)/sizeof(*hue)) << std::endl;

    //std::cout << hue << std::endl;
    
//    for (int i=0; i<1; i++)
//    {
//        std::cout << hue[i] << std::endl;
//    }
