...
void test2() {
    ...
    H5File fi = OpenH5File("tests/rawfile.h5");
    DataSet dase = OpenH5DataSet(fi, "/FullSpectra/MassAxis");
    DataSet dase2 = OpenH5DataSet(fi, "/FullSpectra/SumSpectrum");
    
    float massaxis[249750];
    double sumspectrum[249750];    
    
    ReadH5DataSet(dase, massaxis);
    //ReadH5DataSet(dase2, sumspectrum);
    
    for(int i=0; i<1000;i++)
    {
        cout << massaxis[i] << " " << sumspectrum[i] << endl;
    }
}
